...
export default function UXDesignLesson() {
  const isMobile = useMobile();
  return (
    <Theme>
      <Header />
      <DesignLessonHeader />
      <Container>{isMobile ? <VideoLesson /> : <VideoLesson />}</Container>
      <Footer />
    </Theme>
  );
}

export function UXDesignMicroLesson() {
  return (
    <Theme>
      <DesignLessonHeader />
      <VideoLesson />
...
export function DesignLessonHeader() {
  const isMobile = useMobile();
  const query = `{
        uiuxDesignCollection {
          items {
          image {
            url
          }
  ...
  const { loading, data } = useContentful("uiuxDesign", query);
  if (loading) {
    return <Loading />;
  }
  return (
    <Theme>
      ...
      {isMobile ? (
        ...
        <Queue backgroundRole="nest" height={64.5}>
          <Container>
            <Grid>
...
export const VideoLesson = () => {
  ...
  const query = `{
    ...
    `;
  const { loading, data } = useContentful("uiuxDesignCourse", query);
